...
* 광고 그룹 "coupon"이 넓은 매치 키워드로 보았을 때, 모바일 기기를 통해 가장 많은 총 클릭수를 기록함, 그 뒤를 이어 "promo"와 "discount"가 따름

<br><br>

<img src= "https://github.com/LHG-Git/Tableau_Analysis/assets/100845169/f3c5b02a-6ba9-4ca1-870a-dfdcbf590613">

* 정확한 매치 키워드로 보았을 때, 가장 높은 총 전환을 기록한 광고 그룹은 "promo"이며, 그 뒤를 이어 "coupon"가 따랐음, 모바일 기기가 가장 많이 사용됨

* 구문 매치 키워드로 보았을 때, 가장 높은 총 전환은 "promo" 광고 그룹이며, 그 뒤를 이어 "coupon"와 "offer"가 따름, 대부분의 기기는 데스크톱을 통해 사용되었음
...
* 전반적으로, 모바일 기기에서 가장 높은 인상(노출)을 기록함
...
* 전체적으로, 가장 높은 CTR(클릭률)은 광고 그룹 쿠폰이 35.75%, 프로모션은 35.45%, 그리고 할인은 27.45%로 모바일 기기에서 나타났으며, 넓은 매치 키워드와의 일치율이 1:1인 경우 CTR은 38.7%임, 캠페인 월별로 볼 때, 가장 높은 CTR은 9월에 있었음
...
# 📋 결론
* 5개월간의 평균 전환율을 기준으로 보면, 전환율 값은 기준선인 7.97%를 초과하고 있음

* 캠페인 기간 동안의 CTR(클릭률) 값을 기준으로 보면, 평균 CTR은 기준값인 27.21%보다 높음

* 캠페인 기간 동안의 CPC(클릭당 비용) 값을 기준으로 보면, 평균 CPC는 $0.8보다 낮음

* 전반적으로, 총 P&L을 기준으로 보면, 회사는 $73,409의 손실을 겪었음, 이는 캠페인 성과를 평가한 결과, 해당 지표가 업계 평균 기준값을 초과하고 있기 때문에 재평가가 필요하다는 것을 의미, 따라서 회사는 제품 가격 전략을 재평가해야 할 필요가 있음

* 12월에 회사가 인지 단계에 초점을 맞추고자 한다면, 광고 그룹 "Sale"을 사용할 수 있음, 월간 추이를 보면, 광고 그룹 "Sale"이 넓은 매치 키워드를 사용하여 가장 낮은 CPM 값을 가지고 있음
  
* 12월에 회사가 고려 단계에 초점을 맞추고자 한다면, 광고 그룹 "promo"와 "free shipping"을 사용할 수 있음
  
* 12월에 회사가 전환/결정 단계에 초점을 맞추고자 한다면, 광고 그룹 "free shipping"을 사용할 수 있음
